...
function wpm_enqueue_styles() {//Ref au theme parent (style.css)
	wp_enqueue_style( 
		'mota_photo', 
		get_stylesheet_uri()
	);
...
function mota_enqueue_style() { //le css perso (styles/styles.css)
    wp_enqueue_style(
        'styles-perso',
...
function mota_enqueue_script_gallery() {//le js de la gallery
    wp_enqueue_script(
        ...
        wp_localize_script(
            ...
            'galleryData',
            [
                'apiUrl' => get_rest_url(null, 'photos/v1/load'),
            ]
        );
}
